...
export class DuckDatabase {
  ...
  public static deletePurchase(id: string): void {
    const db = this.loadDB();
    const puIdx = db.purchases.findIndex(p => p.id === id);
    if (puIdx === -1) return;
    const pur = db.purchases[puIdx];

    // 1. Reverse stock
    const pIdx = db.products.findIndex(p => p.id === pur.productId);
    if (pIdx !== -1) {
       db.products[pIdx].currentStockKg = Math.max(0, db.products[pIdx].currentStockKg - pur.weightKg);
    ...
    if (pur.creditAmount > 0) {
      ...
      if (sIdx !== -1) {
        db.suppliers[sIdx].outstandingBalance -= pur.creditAmount;
      }
    }

    // 3. Remove purchase
    db.purchases.splice(puIdx, 1);
    
    // 4. Remove ledger entry
    db.supplierLedger = db.supplierLedger.filter(l => l.referenceId !== id);
  ...
  public static deleteSale(id: string): void {
    ...
    sale.items.forEach(item => {
      ...
      if (pIdx !== -1) {
        db.products[pIdx].currentStockKg += item.weightKg;
        db.products[pIdx].lastUpdated = new Date().toISOString();
      }
    });
    ...
    if (sale.customerId && sale.creditAmount > 0) {
      const custIdx = db.customers.findIndex(c => c.id === sale.customerId);
      if (custIdx !== -1) {
        db.customers[custIdx].outstandingCredit -= sale.creditAmount;
      }
    }
    ...
    // 4. Remove Ledger entry
    db.customerLedger = db.customerLedger.filter(l => l.referenceId !== id);
  ...
  public static deleteCustomer(id: string): void {
    ...
    db.customerLedger = db.customerLedger.filter(l => l.customerId !== id);
  ...
  public static deleteExpense(id: string): void {
    const db = this.loadDB();
    const exIdx = db.expenses.findIndex(e => e.id === id);
    if (exIdx === -1) return;
    const exp = db.expenses[exIdx];

    // Revert mortality stock reduction
    if (exp.category === 'mortality' && exp.mortalityKg && exp.mortalityKg > 0 && exp.mortalityProductId) {
      const prodIdx = db.products.findIndex(p => p.id === exp.mortalityProductId); 
      if (prodIdx !== -1) {
        db.products[prodIdx].currentStockKg += exp.mortalityKg;
        db.products[prodIdx].lastUpdated = new Date().toISOString();
      }
    }
    
    db.expenses.splice(exIdx, 1);
    this.saveDB(db);
  }
  ...
  public static addSale(sale: Omit<Sale, 'billNumber'>): Sale {
    ...
    configuredSale.items.forEach(item => {
      ...
      if (pIdx !== -1) {
        ...
        console.error(`Product not found for sale item: ${item.productId}`);
      }
    });
    ...
    if (configuredSale.customerId && configuredSale.creditAmount > 0) {
      ...
      if (custIdx !== -1) {
        ...
        console.error(`Customer not found for credit sale: ${configuredSale.customerId}`);
  ...
  public static addPurchase(pur: Purchase): void {
    ...
    // 3. Update Supplier Ledger if restocked
    if (pur.totalAmount > 0) {
      const sIdx = db.suppliers.findIndex(s => s.id === pur.supplierId);
      if (sIdx !== -1) {
        const oldBal = db.suppliers[sIdx].outstandingBalance;
        const newBal = oldBal + pur.totalAmount - pur.paidAmount;
        db.suppliers[sIdx].outstandingBalance = newBal;

        // Credit: Full restock
        const creditEntry: SupplierLedgerEntry = {
        ...
        db.supplierLedger.unshift(creditEntry);

        // Debit: Payment made (if any)
        if (pur.paidAmount > 0) {
          const debitEntry: SupplierLedgerEntry = {
            ...
            balanceAfter: newBal,
            referenceId: pur.id,
            description: `Payment made for restock of ${pur.productName}`
          };
          db.supplierLedger.unshift(debitEntry);
        }
      }
    }
  ...
  public static addExpense(exp: Expense): void {
    ...
    if (exp.category === 'mortality' && exp.mortalityKg && exp.mortalityKg > 0 && exp.mortalityProductId) {
      // Find the specific product that died
      const prodIdx = db.products.findIndex(p => p.id === exp.mortalityProductId); 
      if (prodIdx !== -1) {
        db.products[prodIdx].currentStockKg = Math.max(0, db.products[prodIdx].currentStockKg - exp.mortalityKg);
        db.products[prodIdx].lastUpdated = new Date().toISOString();
      }
    }
