...
export default function Expenses({
  ...
  const [mortalityProductId, setMortalityProductId] = useState<string>('');
  ...
  const liveBirdPrice = useMemo(() => {
    const liveBird = products.find(p => p.name.toLowerCase().includes('live'));
  ...
  const handleCategoryChange = (cat: ExpenseCategory) => {
    ...
    // Removed automatic reset of amount/description for other categories
  };
  ...
  const handleAddExpense = (e: React.FormEvent) => {
    ...
    const payload: Expense = {
      ...
      mortalityProductId: mortalityProductId
    };
            ...
            <form onSubmit={handleAddExpense} className="space-y-4" id="log_expense_form">
              ...
              {category === 'mortality' && (
                <div className="bg-rose-50 border border-rose-100 p-3.5 rounded-xl space-y-3 animate-fade-in" id="mortality_spec_card">
                  ...
                  <div className="space-y-1">
                      <label className="text-[9px] font-bold text-slate-500">Affected Product</label>
                      <select 
                        className="w-full bg-white border border-rose-200 rounded-lg p-2 text-xs font-bold focus:outline-none"
                        value={mortalityProductId || ''}
                        onChange={(e) => setMortalityProductId(e.target.value)}
                        id="select_mort_product"
                      >
                  ...
                  <p className="text-[10px] text-rose-700 leading-relaxed font-semibold">
                    * Automated inventory write-off: Deducts from selected product stock.
                  </p>
                </div>
              )}
